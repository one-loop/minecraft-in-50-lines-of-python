...
app, grass_texture, stone_texture, brick_texture, sky_texture, arm_texture, punch_sound, block_pick = Ursina(), load_texture('assets/grass_block.png'), load_texture('assets/stone_block.png'), load_texture('assets/brick_block.png'), load_texture('assets/skybox.png'), load_texture('assets/arm_texture.png'), Audio('assets/punch_sound', loop=False, autoplay=False), 1 # import textures and sound files
def update():
    global block_pick
...
for z in range(20):
    for x in range(20): voxel =  Voxel((x, 0, z))
player, sky, hand = FirstPersonController(), Sky(), Hand()
app.run()
